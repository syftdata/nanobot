...
def _usage_for_response(agent_loop: Any) -> dict[str, int] | None:
...
def _sse_tool_step(payload: dict[str, Any]) -> bytes:
    """Format a ``tool_step`` SSE frame for API streaming clients."""
...
async def handle_chat_completions(request: web.Request) -> web.Response:
        ...
        async with session_lock:
            try:
                ...
                if not response_text or not response_text.strip():
                    ...
                    if isinstance(mt_content, str) and mt_content.strip():
    ...
    return web.json_response(
        _chat_completion_response(response_text, model_name, _usage_for_response(agent_loop))
    )
