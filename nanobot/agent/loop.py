...
class AgentLoop:
    ...
    async def reload_mcp(self) -> None:
        """Reload MCP servers from config and reconnect stale tool wrappers."""
        ...
        try:
            ...
            logger.warning("MCP reload could not read config: {}", exc)
            return

        connections = list(self._mcp_stacks.items())
        ...
        for name, connection in connections:
            try:
                ...
                if task_is_cancelling():
                    raise
                logger.debug("MCP server '{}' cleanup error during reload (can be ignored)", name)
            except (RuntimeError, BaseExceptionGroup):
                logger.debug("MCP server '{}' cleanup error during reload (can be ignored)", name)

        self.tools.unregister_mcp_tools()
        self._mcp_servers = next_servers
        self._mcp_connecting = False
        self._mcp_closing = False
        try:
            connected = await mcp_tools.connect_mcp_servers(self._mcp_servers, self.tools)
        except asyncio.CancelledError:
            if task_is_cancelling():
                raise
            logger.warning("MCP reload connection cancelled")
            connected = {}
        except Exception as exc:
            logger.warning("MCP reload failed to connect servers: {}", exc)
            connected = {}
        ...
        try:
            async with lock:
                ...
                if on_tool_step is not None:
                    kwargs["on_tool_step"] = on_tool_step
