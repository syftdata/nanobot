...
class SlackChannel(BaseChannel):
    ...
    async def start(self) -> None:
        ...
        if not self.config.bot_token:
            self.logger.error("bot_token not configured")
            return
    ...
    async def _start_socket_mode(self) -> None:
        ...
        if not self.config.app_token:
            self.logger.error("app_token required for socket mode")
            return
    ...
    async def stop(self) -> None:
        ...
        if self._webhook_runner:
            try:
                ...
                self.logger.warning("webhook runner cleanup failed: {}", e)
    ...
    async def send(self, msg: OutboundMessage) -> None:
        ...
        try:
            ...
            if (progress_event and progress_event.tool_hint) or (msg.metadata or {}).get("_tool_hint"):
                if self.config.typing_status and thread_ts:
                    status_text = (
                        (progress_event.tool_hint_label if progress_event else None)
                        or (msg.metadata or {}).get("_tool_hint_label")
                        or msg.content
                        or ""
                    )
            ...
            is_progress = progress_event is not None or (msg.metadata or {}).get("_progress", False)
    ...
    async def _process_slack_event(self, payload: dict) -> None:
        ...
        if (
            ...
            thread_ts = event_ts
        # Show a processing indicator: an assistant typing status when
        # configured, otherwise an :eyes: reaction on the triggering message.
        if self.config.typing_status:
            await self._set_typing_status(
                chat_id, thread_ts or event_ts, self.config.typing_status
            )
        else:
            try:
    ...
    def _format_thread_context(self, messages: list[dict[str, Any]], *, current_ts: str | None) -> list[str]:
        ...
        for item in messages:
            ...
            is_bot = sender == self._bot_user_id or sender in self._all_bot_user_ids
    ...
    async def _set_typing_status(self, channel_id: str, thread_ts: str | None, status: str) -> None:
        ...
        try:
            ...
            self.logger.debug("assistant_threads_setStatus failed: {}", e)
    ...
    async def _fetch_thread_context(self, channel: str, thread_ts: str) -> str | None:
        ...
        if not self._web_client:
            self.logger.debug("No web client, skipping thread context fetch for {}:{}", channel, thread_ts)
            return None
        try:
    ...
    def _strip_bot_mention(self, text: str) -> str:
        ...
        if not text or not bot_ids:
            return text
        for bot_id in sorted(bot_ids, key=len, reverse=True):
