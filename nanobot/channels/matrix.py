...
# mistune >=3.3.3 rewrites URLs with unrecognized schemes to "#harmful-link",
# which silently broke Matrix media: images live on mxc:// URIs. Allow mxc://
# through the RENDERER only — the nh3 cleaner below still enforces that an img
# src must be mxc:// (and <a href> must be https/http/matrix/mailto), so the
# sanitization guarantees are unchanged. javascript:/data: etc. stay blocked.
